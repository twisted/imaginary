...
class PlayerCreationDial(Thing):
    ...
    def verb_turn(self,sentence):
        try:
            val = float(sentence.indirectString('to'))
        except ValueError:
...
class PlayerCreationMachine(Thing):
    ...
    def create(self):
        ...
        self.tube.surface=1
        self.tube.displayName='long glass tube'
    ...
    def button_randomize(self):
        ...
        self.place.allHear(self,' makes a large buzzing sound')
    ...
    def button_generate(self):
        ...
        try:
            ...
            p = Player(nam)
            p.synonyms = ['uup']
            p.location = self.tube
            self.person = p
        else:
            self.place.allHear(self.tube,' lurches sideways and then re-adjusts itself.')
    person=None
    player_name=""
    
    def button_release(self):
        if self.person:
            self.person.location = self.place
            self.place.allHear(self.person," floats through the wall of the glass tube and slumps to the floor.")
            del self.person
        else:
            self.place.allHear("You hear a faint sloshing sound.")
            
    def verb_type(self,sentence):
        s = sentence.directString()
        sentence.subject.place.oneHears(
            sentence.subject,
    ...
    def machine_update(self):
        ...
        print repr((strength, dexterity, endurance, memory, agility, psyche))
    ...
    def verb_kick(self, sentence):
        self.place.oneHears(
            sentence.subject,
            to_subject=("You give the machine a swift kick."),
