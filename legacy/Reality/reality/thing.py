...
"""Basic Reality simulation classes.
"""
...
# Twisted Imports
from twisted.python import observable, reflect, components
...
class Event(reflect.Settable):
    """Event()
    Essentially a dictionary -- this encapsulates the idea of an event
...
class IThing(components.Interface):
    """Hah.  I'm not cataloguing all those methods twice.
    """

class IHearable(components.Interface):
    """I'm an interface which specifies the string formatting of an object when
    it is 'heard' by a TR player.
    """

    def hearPhrase(self, target):
        """Return a phrase that the given target Thing should hear.
        """

def _getAnyAdapter(obj, interface, default=None):
    """Try to get a component from an object.  If that fails, try to get an
    adapter for it.  If that still fails, just return the default given.
    """
    if hasattr(obj, "getComponent"):
        n = obj.getComponent(interface)
        if n is None:
            return default
        else:
            return n
    else:
        return components.getAdapter(obj, interface, default)
...
class Thing(observable.Publisher,
    ...
    """Thing(name[,reality])
    """
    # We appear to implement no interfaces yet (but this is good to have anyway)
    __implements__ = IThing, IHearable

    # Most things aren't doors; they don't "go" anywhere.
    destination = None
    ...
    def __init__(self, name, reality=''):
        ...
        @param name: a string, which will be both the unique keyed name of this
        object, and the displayed name.

        @param reality: an instance of a Reality.realities.Reality, or None.
        If it is unspecified, the reality of this object will be set to the
    ...
    def sameAs(self, other):
        """sameAs(self, other) -> 0 or 1
        
        returns 1 if self can be considered the same as other for 
        gameplay purposes, 0 otherwise
        """
        return self == other
    ...
    def set_reality(self, reality):
        ...
        if self.reality is not None:
            self.reality.removeThing(self)
        if reality is not None:
            reality.addThing(self)
    ...
    def set_name(self, name):
        ...
        if self.reality is not None:
            self.reality.updateName(self,oldname,name)
    ...
    def presentPhrase(self, observer):
        ...
        if location is None:
            return "%s is nowhere" % self.capNounPhrase(observer)
    ...
    def get_wornAppearance(self):
        import clothing
        return self.getComponent(clothing.IWearTarget).wornAppearance
    ...
    def format(self, persplist):
        ...
        would yield: 'You dance with the mop.' """

        if isinstance(persplist,str):
            return persplist
        elif persplist is None:
            return ""
        elif hasattr(persplist, "getComponent"):
            # TODO: decouple formatting into a separate interface
            t = persplist.getComponent(IHearable)
            if t:
                return self.format(t.hearPhrase(self))
            print "Warning!  Componentized doesn't have hearable.", persplist
        elif callable(persplist):
            return self.format(persplist(self))
        elif isinstance(persplist, list) or isinstance(persplist, tuple):
            if persplist and reflect.isinst(persplist[0],Thing):
                persplist = list(persplist)
                persplist[0] = persplist[0].capNounPhrase
            x = StringIO.StringIO()
            for i in persplist:
                x.write(self.format(i))
            return x.getvalue()
        print "Warning!  Don't really know how to format %s" % persplist
        return str(persplist)
    ...
    def pairHears(self,
        ...
        if type(to_other)   not in (types.TupleType,types.ListType):
            to_other   = (to_other,)
        if subject is not None and self in subject.allContainers:
            subject.hears(*to_subject)
        if target is not None and self in target.allContainers:
    ...
    def allHear(self, *args):
        ...
        | room.allHear(disembodied_voice," says 'hello'.")
    ...
    def oneHears(self, subject, to_subject, to_other):
        ...
        | room.oneHears(sentence.subject,
    ...
    def execute(self, sentencestring):
        ...
        """
        from reality import phrase
        self.getComponent(phrase.IParsing).parse(sentencestring)
    ...
    def getAbility(self, verbString):
        ...
        """
        return self.skills.get(verbString, getattr(self, 'ability_'+verbString, None))
    ...
    def reFocus(self):
        ...
        if self._hasIntelligence():
            if self.place:
                ...
                print "no place", self, self.focus, self.location, self.place
    ...
    def set_surface(self, surf):
        ...
        """
        assert surf == True or surf == False, '"Surface" should be a boolean.'
    ...
    def find(self, name):
        ...
        if ll == 1:
            ...
            for i in lst[1:]:
                if not lst[0].sameAs(i):
                    raise error.Ambiguity(name, lst)
            return lst[0]

    def lookAroundFor(self, word, interface=None):
        """
        Thing.lookAroundFor(word [,interface]) -> [Component, Component, ...]
        
         Tell this Player to locate a Thing by a synonym it may be
         known by.
        
         If an interface is given, only return components adhering to that
         interface (which may be Things or adapters for Things).  """

        stuff = []
        if word and word[0] == '$':
            # "Completely invisible" things have names that start with $.  The
            # player can never refer to them.  This is incredibly lame but I'm
            # not taking it out (yet) because I can't remember where the
            # requirement came from.  It seems like there might be some
            # "security" purposes.  At the very least there ought to be a
            # bit. -glyph
            return stuff
        elif word == 'here':
            stuff = [self.location]
        ...
        if len(stuff) > 1:
            newstuff = [stuff[0]]
            for i in stuff:
                if not stuff[0].sameAs(i):
                    newstuff.append(i)
            stuff = newstuff
        if interface:
            return [x for x in
                    [x.getComponent(interface) for x in stuff]
                   if x]
        else:
            return stuff

    def locate(self, name, interface=None):
        objs = self.lookAroundFor(name, interface)
        if len(objs) == 0:
            raise error.CantFind(name)
        elif len(objs) == 1:
            return objs[0]
        else:
            raise error.Ambiguity(name, objs)
    ...
    def __getstate__(self):
        ...
        if dict.has_key('code_space'):
            ...
            #    del cs['__builtins__']
            #    dict['code_space'] = cs
            # programmers are too smart for me
            dict['code_space'] = {'self': self, 'reality': reality}
        return dict
