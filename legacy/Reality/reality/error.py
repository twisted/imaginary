...
class Failure(RealityException):
    ...
    Reality-oriented operation.
    """
    def __init__(self,*args,**kw):
        apply(RealityException.__init__,((self,)+args),kw)
        raise self

class Already(RealityException):
    """
    This exception is raised when an object is being asked to transition to a
    state that it's already in (a locked door to lock, a closed box to close).
    It is often safe to ignore from an AI's perspective but it provides nice
