...
/**
 * This verb allows you to wear and remove Clothing. Clothing should
 * have this verb, one or more clothing locations ("clothing
 * location", "clothing location 2", "clothing location 3", etc.)
 * chosen from the location list in the code, and optionally may want
 * to have a "clothing appearance" String property.
 * 
 * You can only layer up to three pieces of clothing on one location,
 * and clothing will currently only be listed in your description if
 * it is at the top of at least one location. It will be listed as
 * it's current apparent name, or the "clothing appearance" string if
 * available. See the docs for more info on how to use clothing... But
 * remember, <B>If you are going to change any properties other than
 * description or appearance, take the clothes off first.</B> It's a
 * complicated process, and changing the other properties while the
 * clothing is on will probably cause it get to get stuck there in a
 * really annoying and complicated way, requiring a lot of
 * killpropping and scrutinizing and grind several years off your
 * life. Trust us. When in doubt, take it all off first.  ;-)
 *
 * @author Glyph */

public class WearRemove extends Verb
{
