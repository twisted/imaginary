...
from imaginary.iimaginary import (
    INameable, ILitLink, IThing, IVisible, IObstruction,
    IElectromagneticMedium, IDistance, IPathNavigator, IRetriever, IExit)
...
class Idea(record("delegate linkers annotators")):
    ...
    def obtain(self, retriever, navigator):
        """
        ...
        the given L{IPathNavigator}'s C{shouldKeepGoing} returns L{True}.
        ...
        @param navigator: an object which determines which L{Link}s should be
            inspected.

        @type navigator: L{INavigator}

        @return: a generator which yields the results of C{retriever.retrieve}
            which are not L{None}.
        """
        return ObtainResult(self, retriever, navigator)


    def _doObtain(self, retriever, navigator, path, reasonsWhyNot):
        """
        ...
        objections = set(navigator.objectionsTo(finalPath, result))
        ...
        for link in self._applyAnnotators(self._allLinks()):
            ...
            if subpath.isCyclic():
                continue
            if navigator.shouldKeepGoing(subpath):
                for obtained in link.target._doObtain(retriever, navigator,
                                                      subpath, reasonsWhyNot):
                    yield obtained



class ObtainResult(record("idea retriever navigator")):
    """
    ...
    @ivar navigator: The L{INavigator} that L{Idea.obtain} was invoked with.
    ...
    def __iter__(self):
        ...
        """
        reasonsWhyNot = set()
        for result in self.idea._doObtain(self.retriever, self.navigator,
                                          None, reasonsWhyNot):
            yield result
...
class Proximity(record('distance')):
    """
    L{Proximity} is a navigator which will continue traversing any path which is
    shorter than its proscribed distance, but not any longer.
    """
    ...
    def shouldKeepGoing(self, path):
        """
        Implement L{IPathNavigator.shouldKeepGoing} to stop for paths whose sum
        of L{IDistance} annotations is greater than L{Proximity.distance}.
        """
    ...
    def objectionsTo(self, path, result):
        """
        L{Proximity} will never object to anything.
        """
        return ()
...
class Reachable(object):
    ...
    def shouldKeepGoing(self, path):
        """
        L{Proximity} will never stop traversal.
        """
        return True
...
class Visibility(record('observer', observer=None)):
    """
    L{Visibility} is a navigator which will continue traversing any path which
    does not have any obstructions which are opaque to its observer.
    """
    ...
    def shouldKeepGoing(self, path):
        """
        Implement L{IPathNavigator.shouldKeepGoing} to not keep going on links
        that are opaque to the observer.
        """
    ...
    def objectionsTo(self, path, result):
        """
        Implement L{IPathNavigator.objectionsTo} to object to paths which have
...
class Traversability(object):
    ...
    def objectionsTo(self, path, result):
        """
        No objections; logic is all in shouldKeepGoing.
        """
        return ()
...
class And(record('a b')):
    """
    L{And} is a navigator which combines the results of two other navigators,
    and will only traverse those paths which both agree should be traversed.
    """
    ...
    def shouldKeepGoing(self, path):
        """
        Implement L{IPathNavigator.shouldKeepGoing} to keep going for paths
        which both sub-navigators think we should keep going.
        """
        return (self.a.shouldKeepGoing(path) and
                self.b.shouldKeepGoing(path))


    def objectionsTo(self, path, result):
        """
        Combine all objections from both sub-navigators.
        """
        return chain(self.a.objectionsTo(path, result),
                     self.b.objectionsTo(path, result))
...
class Named(record("name retriever observer")):
    """
...
class CanSee(record("providerOf")):
    """
    Wrap a L{Named} or a L{ProviderOf}, yielding the results that it would
    yield, but applying lighting to the ultimate target based on the last
    L{IThing} the path.

    XXX this should really be the job of the L{Visibility} navigator, above.
    """
    ...
    def retrieve(self, path):
        ...
        """
        result = self.providerOf.retrieve(path)
        if result is None:
            return None
